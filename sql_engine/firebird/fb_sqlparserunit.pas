...
var
  FSQLTokens, T, FTblName, TAlterOp, TAlterOpCol, FDefCol,
    ...
    FDefColDropNull, TAlterOpColRestart, TAlterOpColRestart1: TSQLTokenRecord;
begin
  ...
  TAlterOpCol:=AddSQLTokens(stKeyword, TAlterOp, 'COLUMN', []);
    ...
    TAlterOpColRestart:=AddSQLTokens(stKeyword, FColName, 'RESTART', [], 38); //RESTART [WITH startvalue]
      TAlterOpColRestart1:=AddSQLTokens(stKeyword, TAlterOpColRestart, 'WITH', [toOptional]);
      TAlterOpColRestart1:=AddSQLTokens(stInteger, TAlterOpColRestart1, '', [], 39);
...
*)
  T:=AddSQLTokens(stSymbol, [FNotNull, FDefColDropNull, FDefCol1, FDefCol2, FDefCol3, FDefColDrop, FDefColPos, TDropOpConst, TAddOpConstrFL_PK,
     TConstRef6, TConstRef7, TConstRef9, TConstRef8, TAlterOpColRename, TDropField, FGenAlways3, TAddOpConstrCheck1,
     TAlterOpColRestart, TAlterOpColRestart1], ',', [toOptional], 5);
...
begin
  case AChild.Tag of
    ...
    38:if Assigned(FCurOperator) then
          FCurOperator.AlterAction:=ataAlterColumnReset;
    39:if Assigned(FCurOperator) then
          FCurOperator.ParamValue:=AWord;
...
begin
  ...
  begin
    case OP.AlterAction of
      ...
      ataAlterColumnReset:
        begin
          if OP.ParamValue <> '' then
            AddSQLCommandEx('ALTER TABLE %s ALTER COLUMN %s RESTART WITH %s', [FullName, DoFormatName(OP.Field.Caption), OP.ParamValue])
          else
          //RESTART [WITH startvalue]
             AddSQLCommandEx('ALTER TABLE %s ALTER COLUMN %s RESTART', [FullName, DoFormatName(OP.Field.Caption)]);
        end
    else
